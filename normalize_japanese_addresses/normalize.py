...
def normalize(address: str, **kwargs) -> str:
    ...
    addr, pref = normalize_prefecture_names(
        addr = addr, prefectures = prefectures, prefectures_list = prefectures_list, endpoint = endpoint)

    # 市区町村の正規化
    if pref != '' and level >= 2:
        addr, city = normalize_city_names(
            addr = addr, prefectures = prefectures, pref = pref)
...
def get_address_parts(address: str) -> Tuple[str, str, str, str, Optional[float], Optional[float], int]:
    """
    住所の初期設定
    """
    # 初期化
...
def normalize_prefecture_names(addr: str, prefectures: dict, prefectures_list: list, endpoint: str) -> str:
    """
    都道府県名を正規化する
    """
    pref = ''
